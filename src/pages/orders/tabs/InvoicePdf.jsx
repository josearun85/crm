...
export default function InvoicePdf({ invoice, customer, items }) {
  ...
  return (
    <div style={{ fontFamily: 'Inter, Helvetica, Arial, sans-serif', maxWidth: 900, margin: '0 auto', color: '#232323', fontSize: 13, background: '#fff', border: '1px solid #e0e0e0', borderRadius: 12, boxShadow: '0 2px 8px #0001', padding: 0 }}>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e0e0e0', padding: '24px 28px 12px 28px', borderTopLeftRadius: 12, borderTopRightRadius: 12, background: '#fafbfc' }}>
        <div>
          <img src="/logo.png" alt="Sign Company Logo" style={{ height: 90, width: 90, borderRadius: '50%', objectFit: 'cover', border: '1px solid #e0e0e0', background: '#fff', boxShadow: '0 1px 4px #0001' }} />
        </div>
      ...
      {/* Table */}
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 0, fontSize: 13, marginBottom: 0, fontFamily: 'Inter, Helvetica, Arial, sans-serif', letterSpacing: 0.05 }}>
        <thead>
          <tr style={{ background: '#fafbfc', color: '#232323', fontWeight: 600, fontSize: 13 }}>
            <th style={{ border: '1px solid #e0e0e0', padding: 7 }}>S. No.</th>
            <th style={{ border: '1px solid #e0e0e0', padding: 7 }}>Name & Description</th>
            <th style={{ border: '1px solid #e0e0e0', padding: 7 }}>HSN Code</th>
            <th style={{ border: '1px solid #e0e0e0', padding: 7 }}>Qty</th>
            <th style={{ border: '1px solid #e0e0e0', padding: 7 }}>Rate</th>
            <th style={{ border: '1px solid #e0e0e0', padding: 7 }}>Amount</th>
            <th style={{ border: '1px solid #e0e0e0', padding: 7 }}>GST</th>
            <th style={{ border: '1px solid #e0e0e0', padding: 7 }}>Cost After Tax</th>
          </tr>
        </thead>
        <tbody>
          {items.map((item, idx) => {
            ...
            return (
              <tr key={idx} style={{ background: idx % 2 === 0 ? '#fff' : '#fafbfc' }}>
                <td style={{ border: '1px solid #e0e0e0', padding: 6, textAlign: 'center', fontWeight: 500 }}>{idx + 1}</td>
                ...
                <td style={{ border: '1px solid #e0e0e0', padding: 6, textAlign: 'center', fontWeight: 500 }}>{item.hsn_code}</td>
                <td style={{ border: '1px solid #e0e0e0', padding: 6, textAlign: 'center', fontWeight: 500 }}>{item.qty}</td>
                <td style={{ border: '1px solid #e0e0e0', padding: 6, textAlign: 'right', fontWeight: 500 }}>{Number(item.rate).toFixed(2)}</td>
                <td style={{ border: '1px solid #e0e0e0', padding: 6, textAlign: 'right', fontWeight: 500 }}>{amount.toFixed(2)}</td>
                <td style={{ border: '1px solid #e0e0e0', padding: 6, textAlign: 'right', fontWeight: 500 }}>{gstAmount.toFixed(2)} <span style={{ color: '#d32f2f', fontWeight: 600 }}>({gstPercent}%)</span></td>
                <td style={{ border: '1px solid #e0e0e0', padding: 6, textAlign: 'right', fontWeight: 600 }}>{costAfterTax.toFixed(2)}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      {/* Summary */}
      <div style={{ display: 'flex', fontSize: 13, marginTop: 0, background: '#fafbfc', padding: '12px 28px 0 28px', borderBottom: '1px solid #e0e0e0', gap: 18 }}>
      ...
      {/* Footer */}
      <div style={{ borderTop: '1px solid #e0e0e0', fontSize: 13, marginTop: 0, padding: '18px 28px 24px 28px', display: 'flex', background: '#fff', borderBottomLeftRadius: 12, borderBottomRightRadius: 12, gap: 18 }}>
        ...
        <div style={{ flex: 1.2, textAlign: 'center', borderLeft: '1px solid #e0e0e0', paddingLeft: 14 }}>
          ...
          <img src="/qr.png" alt="UPI QR" style={{ height: 100, width: 100, objectFit: 'contain', border: '1px solid #e0e0e0', borderRadius: 8, background: '#fff', marginBottom: 12 }} />
